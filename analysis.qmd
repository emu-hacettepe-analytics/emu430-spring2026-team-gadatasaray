...
```{r}
#| label: prepare-section-2-variables
#| echo: true
#| code-fold: true
...
```{r}
seasonal_xg_accuracy <- core_data_cleaned %>%
...
```{r}
last3seasons_data <- core_data_cleaned %>%
...
```{r}
seasonal_shot_quality <- core_data_cleaned %>%
  group_by(season_short) %>%
  summarize(
...
```{r}
home_away_xg <- core_data_cleaned %>%
  group_by(season_short) %>%
  summarize(
